    function showModal(obj){
      ...
      $('#iframeContent').append('<iframe id="iframe" class="responsive-iframe" src="https://drive.google.com/file/d/'+href+'/preview" ></iframe>');
      $('#iframe').one('load', function(){$('#loadingIframe').empty();});
      
      $('#loadingUnduh').empty();
      $('#loadingUnduh').append('download document...');
      $('#unduhContent').empty();
      $('#unduhContent').append('<a id="unduhContent" href="https://drive.google.com/uc?export=download&id='+href+'" rel="nofollow">UNDUH</a>');
